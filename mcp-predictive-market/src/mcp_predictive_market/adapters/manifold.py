...
class ManifoldAdapter:
    ...
    @staticmethod
    def _extract_tiptap_text(node: dict | str) -> str:
        ...
        for child in node.get("content", []):
            parts.append(ManifoldAdapter._extract_tiptap_text(child))
    ...
    def _parse_market(self, data: dict) -> Market:
        ...
        # Extract description — prefer textDescription, fall back to extracting from Tiptap JSON
        raw_desc = data.get("textDescription") or data.get("description", "")
        description = self._extract_tiptap_text(raw_desc) if isinstance(raw_desc, dict) else str(raw_desc)
